...
export const generateThumnail = async (req: Request, res: Response) => {
  try {
    ...
    let prompt=`Create a ${stylePrompts[style as keyof typeof stylePrompts]} for:${title}` //typescript styling

    //when color_scheme is not null use color_scheme as the key of colorSchemeDescriptions
    if(color_scheme){
      prompt+=`Use a ${colorSchemeDescriptions[color_scheme as keyof typeof colorSchemeDescriptions]}` //typescript styling
    }
