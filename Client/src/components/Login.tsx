...
const Login = () => {
    ...
    const handleSubmit = (e:React.FormEvent<HTMLFormElement>) => {
        e.preventDefault()
        if(state=='login'){
            // calling the backend function login which passes the parameters email and passoword and check the user is valid
            // by using the context Auth
            login(formData)
        }
        else{
            // when user is new then signup function call
            signup(formData)
        }
    }

    // this runs whenever user changes, when component loads check if user exist then navigates to the home page directly
    useEffect(()=>{
        if(user){
            navigate('/')
        }
